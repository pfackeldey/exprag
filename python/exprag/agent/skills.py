...
def render_skill_markdown() -> str:
    ...
    lines = [
        ...
        'Records include `schema_version`. Records with `kind == "run_start"` contain one-time run metadata such as git state, host/process context, and optional user metadata under `value.metadata`; records with `kind == "track"` contain user-tracked JSON values.',
        "",
        "## Workflow",
        "",
        ...
        "6. Use `exprag.select_values(records, path)` only after choosing an exact path from `describe_value_paths`.",
        "7. Use `exprag.track_records(records)` and `exprag.run_start_records(records)` instead of hand-written kind filters.",
        "8. Use plain Python plus `exprag.records_between`, `exprag.get_path`, `exprag.select_values`, and `exprag.latest_records` to answer the user's specific question.",
        "9. Cite `_source_path` and `_line_number` when making claims about a run.",
        "",
        "## Example",
        "",
        "```python",
